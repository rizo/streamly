...
-- large size
{-# INLINE enqueueLIFO #-}
enqueueLIFO :: SVar t m a -> IORef [StreamWR t m a] -> (StreamWR t m a) -> IO ()
...
{-# INLINE enqueueFIFO #-}
enqueueFIFO :: SVar t m a -> LinkedQueue (StreamWR t m a) -> StreamWR t m a -> IO ()
...
enqueueAhead sv q (Right m) = do
    atomicModifyIORefCAS_ q $ \ case
        ([], n) -> ([m], n + 1)  -- increment sequence
        _ -> error "enqueueAhead: queue is not empty"
    ringDoorBell sv
-- Is my understanding correct?
enqueueAhead _ _ (Left _) = error "Encountered RunInIO when the svar is Ahead."
